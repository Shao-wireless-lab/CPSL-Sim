...
classdef plannerClass
    ...
    properties
        ...
        Fs              = 20;
        data            = struct;
        NavStrategy     = 'LawnMower';  % CVT, LawnMower, Custom, Fluxotaxis
        QuantStrategy   = 'NGI'         % DE-CVT, Mod-NGI, MB, NGI, NGI+MB, GDMF
        ccvt_on         = 0;
        ...
        function obj = step(obj,ff,plume)
            ...
            switch obj.NavStrategy
                ...
                case 'CVT' % -- MultiUAS
                    ...
                    switch obj.QuantStrategy
                        case 'GDMF'
                            % -- Gradient Divergence Mass Flux
                            if obj.vehicle(1).vehicleMode == 2
                                x = obj.Position(:,1:2);
                                c = obj.Concentration-1.98;  % -- get enhanced concentration
                                u = obj.WindSpeed(:,1:2);
                                n = obj.numVehicles;
                                cthres = 1;
                                DETECT = 0;
                            ...
                            if obj.vehicle(1).vehicleMode == 3
                                x = obj.Position(:,1:2);
                                c = obj.Concentration-1.98;
                                u = obj.WindSpeed(:,1:2);
                                n = obj.numVehicles;


                                [max_c,idx] = max(c);
                                obj.globalConcPos = [c(idx),x(idx,:)];


                                for i = 1:n
                                    v(i,:) = obj.vehicle(i).v(1:2);
                                end
                                ...
                                k_form = 10;
                                Flim = 15;
                                Fd = (epsilon*D.^(delta-bet)./kappa).^(1/(gam-alph));
                                F = @(r) epsilon*Fd^alph./r.^2 - kappa*Fd^gam./r.^delta;
                                %
                                % -- compute formation vector
                                neighbors = cell(1, n);
                                for i=1:n
                                    neighbors{i} = find(vecnorm(x - x(i, :), 2, 2) < 2 * D);
                                end

                                for i=1:n
                                    my_neighbors = neighbors{i}(neighbors{i} ~= i); % remove the ith agent itself from it's neighbors
                                    p_neighbors = x(my_neighbors, :); % position of neighbors (excluding the ith agent itself)
                                    r = p_neighbors - x(i, :); % matrix of displacements
                                    abs_r = vecnorm(r, 2, 2); % norm of each row vector

                                    % formation force vector

                                    a = epsilon * Fd.^alph * abs_r.^(-bet) - kappa * Fd.^gam * abs_r.^(-delta); % magnitude of accelerations
                                    r_hat = r ./ abs_r; % unit vectors of direction
                                    a_vec_formation = k_form .* sum(r_hat .* a, 1); % acceleration vector

                                    % -taxis vector

                                    % select sensor measurements from neighbors
                                    V_neighbor = u(my_neighbors, :);
                                    C_neighbor = c(my_neighbors);

                                    % select self measurements
                                    C_self = c(i);
                                    V_self = u(i, :);

                                    % -- compute taxis vector
                                    nu = dot(r_hat, V_neighbor, 2); % dot product each corresponding row vector

                                    mass_flux = C_neighbor .* (nu); % negative is influx, positive is outflux

                                    [max_influx,  in_target_neighbor ] = min(mass_flux); % neighbor of highest influx
                                    [max_outflux, out_target_neighbor] = max(mass_flux); % neighbor of highest outflux

                                    athres = -0.1;
                                    if max_influx < 0 - athres% if there are influxes
                                        a_vec_taxis = k_flux * r_hat(in_target_neighbor, :);
                                        %new_strategy = Strategy.fluxotaxis;
                                    elseif max_outflux > 0 + athres % if there are outfluxes
                                        a_vec_taxis = k_flux * r_hat(out_target_neighbor, :);
                                        %new_strategy = Strategy.fluxotaxis;
                                    else % if there are no influxes or outfluxes, fallback to chemotaxis
                                        if size(r_hat, 1) > 0 % if neighbors are present
                                            [C_max, max_agent] = max(C_neighbor);

                                            delta_C = C_max - C_self;

                                            if abs(delta_C) > 0
                                                %new_strategy = Strategy.chemotaxis;
                                                a_vec_taxis = k_chemo * (C_max  - C_self) * r_hat(max_agent, :);
                                            else
                                                cx = obj.globalConcPos;
                                                a_vec_taxis = (cx(2:3)-x(i,:));
                                                %[a_vec_taxis, new_strategy] = casting(parameters);
                                            end
                                        else % fall back to casting
                                            cx = obj.globalConcPos;
                                            a_vec_taxis = (cx(2:3)-x(i,:));
                                            %[a_vec_taxis, new_strategy] = casting(parameters);
                                        end
                                    ...
                                    % force limit on taxis
                                    if norm(a_vec_taxis) > Flim
                                        a_vec_taxis  = a_vec_taxis ./ norm(a_vec_taxis) * Flim;
                                        disp('taxis vec large')
                                    end

                                    % force limit on formation
                                    if norm(a_vec_formation) > Flim
                                        a_vec_formation  = a_vec_formation ./ norm(a_vec_formation) * Flim;
                                        disp('formation vec large')
                                    end
                                    a_vec_tmp = a_vec_formation + a_vec_taxis;

                                    % force limit on summation
                                    if norm(a_vec_tmp) > Flim
                                        a_vec_tmp  = a_vec_tmp ./ norm(a_vec_tmp) * Flim;
                                    end

                                    a_vec(i, :) = a_vec_tmp;

                                    % adding drag
                                    a_vec(i, :) = a_vec(i, :) - k_drag * v(i, :); % a <- a - k_drag * v (also known as linear drag)
                                end

                                dt = obj.dt;
                                A = [[1 0 dt 0 ]
                                    [0 1 0  dt]
                                    [0 0 1  0 ]   % equiv to x_f = x_0 + v_x_0*dt
                                    [0 0 0  1 ]];
                                B = 0.5 * transpose(a_vec) * dt.^2;
                                D = transpose(a_vec) * dt;
                                xv_prime = transpose(A * transpose([x,v]) + [B; D]);
                                for i = 1:obj.numVehicles
                                    % -- take each vehicle's measurements
                                    obj.vehicle(i).p(1:2) = xv_prime(i,1:2);
                                    obj.vehicle(i).v(1:2) = xv_prime(i,3:4);
                                end
                            end
                        otherwise
                    end
