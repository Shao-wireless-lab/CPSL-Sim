...
def save_summary_to_file(summary_dict, scenario_key, filepath="/home/ece213/CPSL-Sim_2/results/test_results/eval_summary.json"):
    ...
    data[scenario_key] = summary_dict  # Overwrite if exists
...
def experiment(config):
    ...
    ppo_config["env_config"]["tolerance"] = 5
    ...
    results = {
        ...
        "timesteps_total": 0,
    "episodes_with_agent_collision": 0,
    "episodes_with_obstacle_collision": 0
    }
    episode_centroid_distances = []
    episode_centroid_distances_offset = []
    num_episodes = 0
    ...
    while num_episodes < max_num_episodes:
        ...
        had_agent_collision = False
        had_obstacle_collision = False
        ...
        while not dones["__all__"] and iteration < num_iterations:
            ...
            if agent_collision_sum > 0:
                had_agent_collision = True
            if obstacle_collision_sum > 0:
        ...
        for agent_id, info in infos.items():
            if "final_cent_dist_to_emitter" in info and "final_cent_dist_to_emitter_offset" in info:
                episode_centroid_distances.append(info["final_cent_dist_to_emitter"])
                episode_centroid_distances_offset.append(info["final_cent_dist_to_emitter_offset"])
                break  # Only take from one agent
        num_episodes += 1
        results["num_episodes"] = num_episodes
        if had_agent_collision:
            results["episodes_with_agent_collision"] += 1
        if had_obstacle_collision:
            results["episodes_with_obstacle_collision"] += 1

    # === Final Summary ===
    total_episodes = results["num_episodes"]
    if total_episodes > 0:
        logger.info("======== EVALUATION SUMMARY ========")
        if episode_centroid_distances:
            avg_dist = sum(episode_centroid_distances) / len(episode_centroid_distances)
            avg_offset_dist = sum(episode_centroid_distances_offset) / len(episode_centroid_distances_offset)
            logger.info(f"Average Final Centroid Distance to Emitter: {avg_dist:.3f}")
            logger.info(f"Average Final Centroid Distance to Emitter (Offset): {avg_offset_dist:.3f}")

            if len(episode_centroid_distances) > 1:
                std_dist = statistics.stdev(episode_centroid_distances)
                std_offset_dist = statistics.stdev(episode_centroid_distances_offset)
                logger.info(f"Standard Deviation of Final Centroid Distance: {std_dist:.3f}")
                logger.info(f"Standard Deviation of Final Centroid Distance (Offset): {std_offset_dist:.3f}")
        ...
        logger.info(f"Episodes with Agent Collisions: {results['episodes_with_agent_collision']} / {total_episodes}")
        logger.info(
            f"Episodes with Obstacle Collisions: {results['episodes_with_obstacle_collision']} / {total_episodes}")

        logger.info("====================================")

        # === Prepare and Save Summary ===
        summary_data = {
            "total_episodes": total_episodes,
            "avg_agent_collisions_per_episode": results['agent_collisions'] / total_episodes,
            "avg_obstacle_collisions_per_episode": results['obstacle_collisions'] / total_episodes,
            "drop_found_rate": results['drop_found'] / total_episodes,
            "centroid_stable_rate": results['centroid_stable'] / total_episodes,
            "target_found_rate": results['target_found'] / total_episodes,
            "episodes_with_agent_collision": results['episodes_with_agent_collision'],
            "episodes_with_obstacle_collision": results['episodes_with_obstacle_collision'],
        }

        if episode_centroid_distances:
            avg_dist = sum(episode_centroid_distances) / len(episode_centroid_distances)
            avg_offset_dist = sum(episode_centroid_distances_offset) / len(episode_centroid_distances_offset)
            summary_data["avg_final_centroid_distance"] = avg_dist
            summary_data["avg_final_centroid_distance_offset"] = avg_offset_dist

            if len(episode_centroid_distances) > 1:
                std_dist = statistics.stdev(episode_centroid_distances)
                std_offset_dist = statistics.stdev(episode_centroid_distances_offset)
                summary_data["std_final_centroid_distance"] = std_dist
                summary_data["std_final_centroid_distance_offset"] = std_offset_dist

        scenario_key = ppo_config["env_config"]["scenario_level"]
        save_summary_to_file(summary_data, scenario_key)
